...
connection {
  ...
  provisioner "local-exec" {
    when    = "destroy"
    command = "scp -i ${var.vm_private_key_file} ${local.ssh_options} ${path.module}/lib/destroy/delete_worker.sh ${var.ssh_user}@${local.icp_boot_node_ip}:/tmp/"
  }
  provisioner "local-exec" {
    when    = "destroy"
...
resource "null_resource" "copy_delete_worker" {
  ...
  provisioner "file" {
    source = "${path.module}/lib/destroy/delete_worker.sh"
